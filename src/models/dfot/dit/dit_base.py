    ...
    def __init__(
        ...
        Args:
            ...
            variant: Variant of the DiT model to use.
                ...
                - "factorized_encoder": alternate between spatial transformer blocks
                    and temporal transformer blocks
                - "factorized_attention": decompose the multi-head attention in
                the transformer block, compute spatial self-attention and then temporal self-attention  # noqa: E501
            pos_emb_type: Type of positional embedding to use.
                ...
                - "sinusoidal_factorized": sinusoidal 2D positional embeddings for
                    spatial and 1D for temporal
    ...
    def forward(self, x: torch.Tensor, c: torch.Tensor) -> torch.Tensor:
        ...
        if x.size(1) > self.max_tokens:
            if not self.training or self.num_patches is None:
                raise ValueError(
                    f"Input sequence length {x.size(1)} exceeds the maximum "
                    f"length {self.max_tokens}"
                )
