...
class StochasticUnknownTimesteps(Timesteps):
    ...
    def forward(self, timesteps: torch.Tensor, mask: Optional[torch.Tensor] = None):
        ...
        # training or mask is None - randomly replace embeddings with unknown token
        # with probability p
        # (mask can only be None for logging training visualization when using latents)
        # or if p == 1.0 - always replace embeddings with unknown
        # token even during inference)
        ...
        # # inference with p < 1.0 - replace embeddings with unknown token only for
        # masked timesteps
        # if mask is None:
...
class FourierEmbedding(torch.nn.Module):
    """
    Adapted from EDM2 - https://github.com/NVlabs/edm2/blob/38d5a70fe338edc8b3aac4da8a0cefbc4a057fb8/training/networks_edm2.py#L73  # noqa: E501
    """
...
def get_timestep_embedding(
    ...
    """
    This matches the implementation in Denoising Diffusion Probabilistic Models: Create
    sinusoidal timestep embeddings.

    :param timesteps: a 1-D or 2-D Tensor of N indices, one per batch element. These may
                      be fractional.
    :param embedding_dim: the dimension of the output.
    :param max_period: controls the minimum frequency of the embeddings.
    :return: an [N x dim] or [N x M x dim] Tensor of positional embeddings.
    """
...
# Copied and adapted from https://github.com/lucidrains/rotary-embedding-torch
def rotate_half(x):
...
class RotaryEmbeddingND(nn.Module):
    ...
    def forward(self, x: torch.Tensor) -> torch.Tensor:
        """
        Args:
            x: a [... x N x ... x D] if flatten=False, [... x (N x ...) x D] if
            flatten=True
            tensor of queries or keys.
        Returns:
...
class RotaryEmbedding3D(RotaryEmbeddingND):
    ...
    def __init__(
        ...
        # split into 3 dimensions such that height and width have the same number of
        # frequencies
        match dim % 3:
            case 0:
                dims = (dim // 3,) * 3
...
class RandomEmbeddingDropout(nn.Module):
    ...
    def forward(self, emb: torch.Tensor, mask: Optional[torch.Tensor] = None):
        """
        Randomly nullify the input embeddings with a probability p during training. For
        inference, the embeddings are nullified only if mask is provided.
        Args:
            emb: input embeddings of shape (B, ...)
            mask: mask tensor of shape (B, ). Only allowed during inference. If
            provided, embeddings for masked batches will be zeroed.
        """
        if mask is not None:
...
class RandomDropoutCondEmbedding(TimestepEmbedding):
    """
    A layer for processing conditions into embeddings, randomly dropping embeddings of
    each frame during training.
    NOTE: If dropout_prob is 0, it will fall back to `TimestepEmbedding`. We use this
    trick to ensure the backward compatibility with our previous checkpoints.
    """
